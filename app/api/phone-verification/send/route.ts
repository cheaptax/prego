...
import { getClientIpHash } from "@/lib/audit-quote/security";
import { adminDb } from "@/lib/firebase/admin";
import {
  isPhoneOtpPurpose,
} from "@/lib/phone-verification/otp";
import { sendPhoneOtpChallenge } from "@/lib/phone-verification/service";
...
export async function POST(req: Request) {
  ...
  try {
    body = JSON.parse(guarded.rawBody) as { phone?: unknown; purpose?: unknown };
  } catch {
    return NextResponse.json({ ok: false, error: "invalid_json" }, { status: 400 });
  }
  if (!isPhoneOtpPurpose(body.purpose) || typeof body.phone !== "string") {
    return NextResponse.json(
      { ok: false, error: "invalid_input" },
      { status: 400 },
    );
  }
  const result = await sendPhoneOtpChallenge({
    db: adminDb(),
    phone: body.phone,
  ...
  return NextResponse.json(
    result.localCode
      ? { ok: true, localCode: result.localCode }
      : { ok: true },
  );
}
